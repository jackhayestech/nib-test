...
import { OrderDisplay, ProductStockLevels } from "../interfaces";
...
export class OrderManagement {
  ...
  displayOrdersStatuses = (): void => {
    const orders: OrderDisplay[] = this.orders.map(({ orderId, status }) => ({
      orderId,
      status,
    }));
  ...
  displayProductStockLevels = (): void => {
    const productStock: ProductStockLevels[] = this.products.map(
      ({ productId, quantityOnHand }) => ({ productId, quantityOnHand })
    );
